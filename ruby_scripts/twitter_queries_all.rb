...
RECIPIENTS = 	['jenniferh@twitter.com', 'hcomp@twitter.com', 'dino@twitter.com', 'blake.street@crowdflower.com', 'kirsten.gokay@crowdflower.com']
...
def row_usage_query(redshift)
	row_usage_results = redshift.query("
		select t.name, t.id,
		sum(case when a.created_at between (current_date - 7) and (current_date - 1) then number_units end) as last_week
...
def judgments_query(redshift)
	judgments_results = redshift.query("
		...
		COUNT(DISTINCT(CASE WHEN (j.team_id = 'ac0b8c8c-7d58-44e8-b2d7-875e46071fc3' and builder_judgments.created_at between (current_date - 7) and (current_date - 1)) THEN builder_judgments.id END)) AS abuse_thisweek,
		COUNT(DISTINCT(CASE WHEN (j.team_id = '30a242db-54d2-43fd-b112-96e638bc1031' and builder_judgments.created_at between (current_date - 7) and (current_date - 1)) THEN builder_judgments.id END)) AS content_thisweek,
		COUNT(DISTINCT(CASE WHEN (j.team_id = 'e2b56b17-aa70-491e-a0ec-8ae93a5e4cb9' and builder_judgments.created_at between (current_date - 7) and (current_date - 1)) THEN builder_judgments.id END)) AS cortex_thisweek,
		COUNT(DISTINCT(CASE WHEN (j.team_id = '5e1a604e-372e-4761-86f5-cf61b6670de6' and builder_judgments.created_at between (current_date - 7) and (current_date - 1)) THEN builder_judgments.id END)) AS spam_thisweek,
...
def agreement_query(redshift)
	agreement_results = redshift.query("select 
		...
		where ju.created_at between (current_date - 7) and (current_date - 1)
		and j.team_id in ('ac0b8c8c-7d58-44e8-b2d7-875e46071fc3',
		'30a242db-54d2-43fd-b112-96e638bc1031',
		'e2b56b17-aa70-491e-a0ec-8ae93a5e4cb9',
		'5e1a604e-372e-4761-86f5-cf61b6670de6')
