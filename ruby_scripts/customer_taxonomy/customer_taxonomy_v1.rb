...
client = Restforce.new(   
  ...
  password:       '45CoolPassword',
  security_token: 't8FCRz7P1nfTQK7VaYqIBJq6m',
...
csv_headers =["salesforce_pn", "salesforce_account", "sf_last_work_order", "sf_fee_type", "contract_exp_date", "builder_organizations_pn",
  ...
  summary_headers = ["PN","organization_name","active_users", "number_of_launched_jobs", "total_crowdspend_with_markup", "total_finalized_units"]
      ...
      #Iterate through SF Hash to populate redshift query 
        my_hash.each do |pn, array|
          ...
          rs_query_build_org = redshift.query("
            ...
            JOIN (Select MAX(builder_conversions.created_at) as last_conversion, 
          ...
          # if build_jobs query is not empty, fill csv
            if rs_query_build_jobs.ntuples > 0
              ...
              summary_report << [pn, sf_name, "PN Mismatch"]
                ...
                full_report << [pn, sf_name, last_work_order, fee_type, contract_exp_date, "No Units Processed"]
                summary_report << [pn, sf_name, "No Units Processed"]
                # removed from pn_mismatch report
                # pn_mismatch_report << [pn, sf_name, last_work_order, fee_type, contract_exp_date, "Not Found", "No Units Processed"] 
