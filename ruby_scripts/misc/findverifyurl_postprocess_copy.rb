    ...
    reader.each do |row|
      ...
      # if it isn't verified
        ...
        if row["url_unverified_found"].to_s == "yes_corrected" or row["url_unverified_found"].to_s == "yes_noaddress"
          if row["url_unverified_text"] != "" # should be url_unverified_text_worker_input
            url_cf = row["url_unverified_text"] # should be url_unverified_text_worker_input
          end
        end
      ...
      # check if url found if provided url not provided or is wrong
        ...
        if row["url_found_text"] != "" # should be url_unverified_text_worker_input
          url_cf = row["url_found_text"] # should be url_unverified_text_worker_input
        end
      end
