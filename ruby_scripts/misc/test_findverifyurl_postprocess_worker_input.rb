# Post-processing script for Bloomberg URL Job 1 output
# Input: 1) agg csv
...
headers_first_row = headers_first_row + ["url_cf"]
...
CSV.open(output1_csv, 'w', headers: headers_first_row, write_headers: true) do |new_csv|
  CSV.open(output2_csv, 'w', headers: headers_first_row, write_headers: true) do |end_csv|
    reader.each do |row|
      ...
      # check if provided url is verified
      url_cf = ""
      # if it is verified
      if row["url_verified"].to_s == "yes_verified" or row["url_verified"].to_s == "yes_noaddress"
        url_cf = row["company_web_address"]
      # if it isn't verified
      elsif row["url_verified"].to_s == "no"
        if row["url_unverified_found"].to_s == "yes_corrected" or row["url_unverified_found"].to_s == "yes_noaddress"
          if row["url_unverified_text_worker_input"] != "" # should be url_unverified_text_worker_input
            url_cf = row["url_unverified_text_worker_input"].to_s # should be url_unverified_text_worker_input
          end
        end
      
      # check if url found if provided url not provided or is wrong
      elsif row["url_found"].to_s == "yes_corrected" or row["url_found"].to_s == "yes_noaddress"
        if row["url_found_text_worker_input"] != "" # should be url_unverified_text_worker_input
          url_cf = row["url_found_text_worker_input"].to_s # should be url_unverified_text_worker_input
        end
      end

      if url_cf == ""
        end_csv << headers_to_write.map { |h| row[h]} + [url_cf]
        blank_urls += 1
      else
        if !url_cf.include?("http")
          url_cf = "http://" + url_cf
        end
        rows_written += 1
        new_csv << headers_to_write.map { |h| row[h]} + [url_cf]
      end
...
p "Units with Blank URLs: #{blank_urls}"