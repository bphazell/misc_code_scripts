...
wordorder_postprocess <- function(sour,df){
  ...
  #check na logic
  write.csv(df, "check_na_logic_wordorder.csv", row.names=F)
  ...
  new_vector = gsub(", $","",new_vector)
  df$word_order = new_vector
  df = df[names(df) %in% c("unique_id", "conceptname", "top10children", "word_order")]
  output = merge(sour, df)
  output = output[order(output$unique_id),]
  write.csv(output, "wordorder_postprocessed.csv",row.names=F)
  return(output)
}

test = wordorder_postprocess(sour,agg)
