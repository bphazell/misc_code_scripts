...
sour = read.csv("Source/source_for_dedupe_incomplete_wordorder.csv", stringsAsFactors=F)

agg = read.csv("Dedupe/dedupe_a413486.csv", stringsAsFactors=F)



dedupe_postprocess <- function(sour,df){
  ...
  #check na logic
  write.csv(df, "check_na_logic_dedupe.csv", row.names=F)
  df = df[names(df) %in% c("conceptname", "del1", "del2", "newdel1","newdel2","top10children","unique_id", "new_agg","new_agg2")]
  df$newdel1 = NULL
  df$newdel2 = NULL
  colnames(df) = c("conceptname", "del1", "del2","top10children","unique_id", "part1","part2")
  ...
  new_vector = gsub(", $","",new_vector)
  new_vector_test = as.data.frame(new_vector)
  df$dedupe = new_vector
  df = df[names(df) %in% c("unique_id", "conceptname", "top10children", "dedupe")]
  output = merge(sour, df)
  output = output[order(output$unique_id),]
  write.csv(output, "dedupe_postprocessed.csv",row.names=F)
  return(output)
}

test =dedupe_postprocess(sour,agg)
