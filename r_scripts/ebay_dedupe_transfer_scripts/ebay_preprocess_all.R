...
#factors are variables catered towards statistical modeling 
#factors are stored in as a vector of integer values with an associated set of character values
...
#    function outputs 2 dataframes:
#          1. dedupe/incomplete/wordorer
#          2. for spellchekcer 


ebay_preprocess = function(sour){
  new_sour = sour
  names(new_sour) = tolower(names(new_sour))
  new_sour$unique_id = ""
  new_sour$del1 = ""
  ...
  write.csv(new_complete, "source_for_dedupe_incomplete_wordorder.csv", row.names=F)
  
  #For Spelling
  spell = data.frame(parent=character(0), child=character(0))
  concept_vector = new_sour[,"conceptname"]
  children_vector = new_sour[,"top10children"]
  spell_vector = paste(concept_vector, children_vector, sep = ", " )
  for (i in 1:nrow(sour)){
    split = unlist(strsplit(spell_vector[i], ", "))
    new = merge(concept_vector[i],split )
    colnames(new) = c("parent","child")
    spell = rbind(spell,new)
    print( paste ("Spelling ",i))
    write.csv(spell, "for_spelling_script.csv", row.names=F)
  } 
  
  return (list(spell,new_complete))
}
