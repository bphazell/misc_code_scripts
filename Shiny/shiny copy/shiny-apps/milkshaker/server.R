...
shinyServer(function(input, output){
  ...
  ###Decide to Multi-Select Parse:
  
  output$multiSelectParse <- renderUI({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id_chosen==0) {
      return(NULL)
    } else{
      checkboxInput(inputId="multi_parse", label="Would you like to account for multi-select fields?")
    }
  })
  ...
  flag_individual_workers <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && (input$job_id_chosen==0)){
      ...
      df=create_work_reject()
      if (nrow(df) == 0) {
        return(NULL)
      } else {
        potential_offenders = unique(df$X_worker_id)
        potential_gets = paste("get_", potential_offenders, sep="")
        ids_to_reject = c()
        for (i in 1:length(potential_gets)) {
          if(input[[potential_gets[i]]] == 0) {
            ...
            # in the future: remove from the flagged list
          } else {
        ...
        for(i in 1:nrow(table)){
          ...
          table$reject[i] = 
            paste('<button class=\"btn btn-danger action-button shiny-bound-input\" 
                 data-toggle=\"button\" id=\"get_', workers[i],
  ...
  output$createSearchTable <- renderDataTable({
    if (is.null(input$files[1]) || is.na(input$files[1])) {
      ...
      #                         $(this).html($(this).text().substring(0, maxChars-4) + '...');
      #                     }
      #             });
      #             return nRow;
      #         } !#"
  ...
  output$answerSelectorMilkshaker <- renderUI({
    if (is.null(input$files[1]) || is.na(input$files[1])) {
      ...
      selectInput("answer_chosen_milkshaker", 
  ...
  output$total_distros <- renderChart({
    if (is.null(input$files[1]) || is.na(input$files[1]) || is.null(input$question_chosen)) {
      #idk why richshaw works instead of Nvd3 but whatever.
      #return(NULL)
      return(Nvd3$new())
    } else {
      ...
      ##PARSING NOT WORKING
      counts = length(answer_cols_names)
      new_file <- 
        as.data.frame(setNames(replicate(counts, numeric(0), simplify = F), letters[1:counts]))
      
      print(names(new_file))
      print("at line 738")
      #       if(input$multi_parse == TRUE){
      #        for(i in 1:length(answer_cols_names)){
      #           colnames(new_file)[i] <- answer_cols_names[i]
      #           print("Made it to the loop, line 745")
      #           print(names(new_file))
      #           answer = full_file[,c(answer_cols_names[i])]
      #           #print(answer)
      #           split = unlist(str_split(answer, "\n"))
      #           print(length(split))
      #           new_file$answer_cols_names[i] = split
      #         }
      #        
      #        print("made it to 745")
      #      #  print(head(new_file))
      #        print(names(new_file))
      #       }
      ...
      responses = lapply(answer_cols_names, function(x) {
        #if(input$multi_parse == FALSE){
        responses = table(full_file[,names(full_file) == x])
        #} else {
        #  responses = table(new_file[names(new_file) == x])
        #}
