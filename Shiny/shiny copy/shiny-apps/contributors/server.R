...
shinyServer(function(input, output, session) {
  ...
  output$stackedGraph <- renderPlot({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && (input$get_file==0)){
      ...
      h = h + theme_bw() + theme(axis.text.x=element_blank(), axis.title.x=element_blank(),
...
profile_golds <- reactive({
  if ((is.null(input$files[1]) || is.na(input$files[1])) && (input$get_file==0) || (input$id_chosen_profiles == "")) {
    ...
    if(input$id_chosen_profiles != ""){
      ...
      worker_answers = worker_answers[order(worker_answers$X_missed, decreasing = T),]
      worker_answers
    }
  }
})
...
output$html_gold_table <- renderText({
  ...
  output$summary_message <- renderText({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && (input$get_file==0)){
      ...
      if (num_gold_units == 0) {
        gold_message = "<p style='color:red;'>It looks like you don't have any <b>Gold units</b> in your file. Change reporting settings and regenerate the report if this is not intentional.</p>"
      } else {
        gold_message = ""
      }
