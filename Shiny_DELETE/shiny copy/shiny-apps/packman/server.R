...
#     new_app_message = paste("<div class=\"alert alert-block\">
#                             <h5>Outstanding:</h5>
...
#   })
  ...
  agg_file <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) {
      ...
      print("In job id")
      id_str = input$job_id
      job_array = unlist(strsplit(id_str, split=","))
      file_list = list()
      ind = 1
      for (inFile in job_array) {
        system(paste('s3cmd --force get s3://crowdflower_prod/a',
                     inFile,'.csv.zip /tmp/a', inFile, '.zip',sep=''),
               intern=T)
        system(paste('unzip -o /tmp/a', inFile, '.zip -d /tmp', sep=''))
        val <- try(read.csv(paste('/tmp/a',inFile,'.csv',sep='')), silent=TRUE)
        if (inherits(val, "try-error")) {
          stop(paste("Sorry, we could not find your file. Either job", inFile,
                     "does not exist or its Agg Report has not been generated yet.",
                     "Check", paste("http://crowdflower.com/jobs/",inFile,"/reports.",sep=""),
                     "\nRefresh this page before uploading a csv."))
        } else {
          agg_file = read.csv(paste('/tmp/a',inFile,'.csv',sep=''),colClasses="character")
          agg_file$app_source_job_id = inFile
          file_list[[ind]] = agg_file
          ind = ind + 1
        }
      }
      agg_file = rbind.fill(file_list)
    } else {
      agg_file = read.csv(input$files$datapath, na.strings="NaN", stringsAsFactors=FALSE, colClasses="character")
    }
    return(agg_file)
  })
  ...
  ##Tab 1 - File Stats: downloadable statistics on original inputted file
  output$selectAggCols <- renderUI({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      default_columns = get_cml_names()
      all_columns = get_names()
      
      selectizeInput("agg_cols_chosen",
                     "Select the columns we should use to generate the file statistics. We've preselected a few. If a column
                     you want is missing start typing it and it'll populate. Delete columns by selecting and pressing 'delete/backspace' 
                     on your keyboard.",
                     choices = all_columns,
                     multiple=TRUE,
  ...
  build_file_stats <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0 || input$get_stats == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      file = agg_file()
      names = input$agg_cols_chosen
  ...
  output$createAggStats <- renderText({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0 || input$get_stats == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      inp =  build_file_stats()
      report_list =inp$r
      ...
      for(i in 1:length(report_list)){
        ...
        if(num_item > 10){
          num_item = 10
          item[10] = "Other Non Empty Values"
          answer[10] = sum(answer[10:length(item)])
        }
  ...
  output$downloadFileStats <- downloadHandler(
    
    filename = function() { paste('uploaded_file_stats', '.csv', sep='') },
    content = function(file) {
      df=build_agg_stats_csv()
  ...
  output$rowProperCase <- renderUI({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0 || input$get_reorder == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
  ...
  ##Dedupe Rows
  row_dedupe <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0 || input$get_clean == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      ...
      list(file = change_file, found_duplicates = length(dup_lists))
    }
  })
  ...
  col_rename <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) {
      ...
      #nothings been decided with the file yet
     if(input$skip_dedupe == F && input$get_dedupe == 0){
        return(NULL)
      } else {
        if(input$skip_dedupe == F){
          change_file = row_dedupe()$file
        }else{
          change_file = row_proper_case()
        }
  ...
  output$dedupeTabTable <- renderTable ({
    if (((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) || input$get_clean == 0) {
      ...
      print(481)
      deduped_list = row_dedupe()
      if(input$get_dedupe != 0){
        print(483)
        table = deduped_list$file
      }
  ...
  output$dedupeTabTableMessage <- renderText ({
    if (((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) || input$get_clean == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      deduped_list = row_dedupe()
      if(input$get_dedupe != 0){
        num_dupes = deduped_list$found_duplicates
      } else {
        num_dupes = 0
      }
      
      if(input$get_dedupe != 0){
        paste("We have found", num_dupes, "duplicate rows based on the columns you provided. Your file has been modified. 
               Go back to the previous tab if you'd like to un-do this.")
      } else {
        paste("*<em>You can skip deduplicaion by checking the box above and continuing to the next tab.</em>")
      }
    }
  })
  ...
  find_missing_units <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1]) ||
           is.null(input$source_file[1]) || is.na(input$source_file[1])) && input$job_id == 0){
      return(NULL)
    } else {
  ...
  output$missingUnits <- renderDataTable({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0 || is.null(input$source_file[1]) || is.na(input$source_file[1])){
      return(NULL)
    } else {
  ...
  output$selectReportCardCols <- renderUI({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) {
      ...
      selectizeInput("report_cols_chosen",
  ...
  build_report_card <- reactive({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0 || input$get_report == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      file = col_rename()
  ...
  output$mixpanelEvent_job_id_packman <- renderText({
    if ((is.null(input$files[1]) || is.na(input$files[1])) && input$job_id == 0) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      if (input$job_id != 0) {
        ...
        print(filename)
        job_ids = gsub(filename, pattern="^a", replacement="")
        job_ids = str_extract(job_ids, "\\d{6}")
        print(job_ids)
      }
      mxp = paste0("<script>mixpanel.track('packman job ids',{job_id: ",
                   job_ids,"})</script>")
      paste(mxp)
    }
  })
  
  output$mixpanelEvent_logic_job_id_packman <- renderText({
    if (is.null(input$files_logic)) {
      # User has not uploaded a file yet
      return(NULL)
    } else {
      if (input$job_id_logic == 0) {
        inFile <- input$files_logic$name
        job_id = gsub(inFile, pattern="^a", replacement="")
        job_id = str_extract(job_id, "\\d{6}")
      } else {
        job_id = input$job_id_logic
      }
      mxp = paste0("<script>mixpanel.track('packman logic job ids',{job_id: ",
                   job_id,"})</script>")
      paste(mxp)
    }
  })
